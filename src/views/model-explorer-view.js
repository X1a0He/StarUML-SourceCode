...
class ModelExplorerView extends EventEmitter {
          ...
          this.$viewContent.animate(
            {
              scrollTop:
                node.offset().top -
                this.$viewContent.offset().top +
                this.$viewContent.scrollTop(),
            },
            500,
          );
        }
  ...
  htmlReady() {
    ...
    this._showStereotype = this.preferenceManager.get(
      "explorer.showStereotype",
      true,
    );

    // Setup UI
    const viewTemplate = fs.readFileSync(
      path.join(__dirname, "../static/html-contents/model-explorer-view.html"),
      "utf8",
    );
