...
ExtensionManagerViewModel.prototype._updateMessage = function () {
  if (
    ...
    this.message =
      this.sortedFullSet && this.sortedFullSet.length
  ...
  return (
    ...
    _searchFields.some(function (fieldSpec) {
      ...
      if (Array.isArray(cur)) {
        ...
        if (
          components.length > 1 &&
          components[1].toLowerCase().indexOf(query) !== -1
        ) {
          return true;
        }
 ...
 */
RegistryViewModel.prototype.source =
  ExtensionManagerViewModel.prototype.SOURCE_REGISTRY;
...
RegistryViewModel.prototype._initializeFromSource = function () {
  ...
  return ExtensionManager.downloadRegistry()
    .done(function () {
      ...
      self.sortedFullSet = RegistryUtils.sortRegistry(
        self.extensions,
        "registryInfo",
      )
 ...
 */
InstalledViewModel.prototype.source =
  ExtensionManagerViewModel.prototype.SOURCE_INSTALLED;
...
InstalledViewModel.prototype._initializeFromSource = function () {
  ...
  this.sortedFullSet = Object.keys(this.extensions).filter(function (key) {
    return (
      self.extensions[key].installInfo &&
