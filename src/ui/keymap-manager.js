...
class KeymapManager {
  ...
  _mapKeycodeToKey(_keycode, key) {
    ...
    if (_keycode >= keycode("0") && _keycode <= keycode("9")) {
      ...
      _keycode >= keycode("numpad 0") &&
      _keycode <= keycode("numpad 9")
    ) {
  ...
  _isEditKey(event) {
    return (
      (event.ctrlKey && event.which === keycode("c")) ||
      ...
      ((event.ctrlKey || event.metaKey) && event.which === keycode("a")) ||
  ...
  setup() {
    ...
    window.document.body.addEventListener(
      ...
      (event) => {
        ...
        if (
          (self._inModalDialog() && self._isEditKey(event)) ||
          (self._inEditMode() && self._isEditKey(event))
        ) {
