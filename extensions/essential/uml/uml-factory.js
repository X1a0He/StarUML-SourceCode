...
function associationClassFn (parent, diagram, options) {
  ...
  if ((options.tailModel instanceof type.UMLClassifier) &&
    ...
    } else if ((options.tailModel instanceof type.UMLClass) && (options.headModel instanceof type.UMLNaryAssociationNode)) {
      // Class에서 NaryAssociationNode로 연결한 경우 : AssociationClassLink를 생성.
      model.classSide = options.tailModel
      model.associationSide = options.headModel
      view.tail = options.tailView
      view.head = options.headView
    } else if ((options.headModel instanceof type.UMLClass) && (options.tailModel instanceof type.UMLNaryAssociationNode)) {
...
app.factory.registerModelAndViewFn('UMLActivityParameterNode', _modelAndViewFn, { precondition: activityPrecondition, field: 'nodes' })
